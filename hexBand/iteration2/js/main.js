...
(function () {
  ...
  const UI = {
    state: null, renderer: null, busy: false, opts: null, sel: null, lastActor: null, handoverPending: false, lastEvents: [], pendingIncoming: new Set(),
    ...
    showIntro(manual) {
      ...
      const ov = this.overlay(`<div class="intro">
        ...
        <div class="intro-item">${ic('hook')}<div><b>Ходите картами.</b> Тапните карту — на поле подсветятся все места, куда она может привести отряд; тапните нужный гекс, и отряд пойдёт сразу. Карта задаёт форму маршрута, направление выбираете вы. За ход можно сыграть сколько угодно карт, минимум одну; затем «Закончить ход».</div></div>
    ...
    afterAction() {
      ...
      this.sel = null; $('#card-desc').hidden = true;
    ...
    // ------------------------------------------------------------ tap to play (iteration2, D-046)
    bindTap() {
      $('#hand').addEventListener('click', e => { const el = e.target.closest('.card'); if (!el || !el.dataset.uid) return; this.onCardTap(+el.dataset.uid, el); });
      $('#card-desc').addEventListener('click', () => this.cancelSelect());
    ...
    showDesc(title, text, needsTarget) {
      const hint = needsTarget ? 'Тапните по подсвеченному гексу на поле, чтобы сыграть' : 'Тапните по полю, чтобы сыграть';
      $('#card-desc').innerHTML = `<div class="desc-body"><b>${title}</b><span>${text}</span></div><div class="desc-cancel"><span class="desc-cancel-ic">✕</span><span>${hint}. Передумали? Тапните по этой панели — карта останется в руке.</span></div>`;
    ...
    cancelSelect() { this.sel = null; $('#card-desc').hidden = true; this.renderer.highlights = []; this.renderer.pathFrom = null; this.refresh(); },
    ...
    commitSel(from) {
      ...
      $('#card-desc').hidden = true; this.renderer.highlights = []; this.renderer.pathFrom = null;
    ...
    onCanvasClick(e) {
      const s = this.state; if (!s) return;
      if (this.sel) { // iteration2: a tap on the board plays the selected card at that target
    ...
    showHelp() {
      const ov = this.overlay(`<div class="help"><h2>Как играть</h2>
        <p><b>Ход.</b> В начале хода рука добирается до 4 карт из колоды (если колода пуста, сброс перемешивается в колоду). Чтобы сыграть карту, тапните её: над рукой появится панель с описанием, а на поле подсветятся все цели. Тапните нужный гекс — отряд выполнит действие сразу, а карта уйдёт в сброс. Карту без цели (Сбор, Боевой клич…) играет тап по полю или повторный тап по карте. Передумали — тапните по панели с описанием, карта останется в руке. За ход можно сыграть сколько угодно карт, минимум одну; после первой карты в правом слоте появляется «Закончить ход».</p>
        <p><b>Направления.</b> Карта движения задаёт только форму и длину маршрута (прямая, крюк, зигзаг, полукольцо). Куда идти — решаете вы: подсвечены все возможные концы маршрута, тап рядом с одним из них выбирает этот вариант (на компьютере маршрут показывается при наведении). Для «Широкого марша» сторона — слева или справа от отряда.</p>
